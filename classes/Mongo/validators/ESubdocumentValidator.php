...
class ESubdocumentValidator extends CValidator
{
	...
	{
		if(!$this->type){
			throw new \Mongo\Exception(Yii::t('yii', 'You must supply a subdocument type of either "many" or "one" in order to validate subdocuments'));
		}
		if(!$this->class && !$this->rules){
			throw new \Mongo\Exception(Yii::t('yii', 'You must supply either some rules to validate by or a class name to use'));
		}
