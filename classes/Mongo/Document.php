...
class Document extends \Mongo\Model
{
	...
	*/
	 ...
	 * function as you would the get\MongoId function. These two functions should never have been separate
	 ...
	 * @return Document
	 */
	protected function instantiate(/** @noinspection PhpUnusedParameterInspection */$document)
	{
	 ...
	 * @param bool $partial
	 * @return Document|null - the newly created active record. The class of the object is the same as the model class.
	 */
	...
	public function insert($attributes = null)
	{
		...
		if(YII_DEBUG){
			...
			//TODO Yii::trace
			var_echo('Executing insert: {$document:' . json_encode($document) . '}', 'extensions.MongoYii.\Mongo\Document');
		}
	...
	public function update($attributes = null)
	{
		...
		if($this->getPrimaryKey() === null){ // An _id is required
			throw new Exception('The active record cannot be updated because it has primary key set.');
		}
	 ...
	 * @param Document $record - record to compare to
	...
	public function findAllByPk($pk, $fields = [])
	{
		...
		if(!is_array($pk)){
			throw new Exception('Set an incorrect primary key.');
		}
	...
	public function find($criteria = [], $fields = [])
	{
		...
		var_echo('Executing find: ' . '{$query:'.json_encode($query) . ',$project:'.json_encode($project)
		...
		if($c !== []){
			$cursor = new Cursor($this, $query, $project);
			if(isset($c['sort'])){
				$cursor->sort($c['sort']);
			}
			if(isset($c['skip'])){
			...
			$cursor = new Cursor($this, $criteria, $project);
		}
		if($this->getDbConnection()->enableProfiling){
			#TODO Add profile
	 ...
	 * @return Document the active record instance itself.
	 */
	 ...
	 * @param array|Criteria $newCriteria
	 * @return array
	 */
	...
	{
		return array_merge($oldCriteria, $newCriteria);
