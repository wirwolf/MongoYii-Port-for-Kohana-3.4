			...
			if(is_array($definition)){
				...
				if(isset($schema)){
					if(($pos = strpos($attribute,'.')) !== false){
						throw new \Mongo\Exception('MongoDB cannot sort on joined fields please modify ' . $attribute . ' to not be sortable');
						...
						//$attribute=($criteria===null || $criteria->alias===null ? \Mongo\Document::model($this->modelClass)->getTableAlias(true) : $schema->quoteTableName($criteria->alias)).'.'.$schema->quoteColumnName($attribute);
					}
				}
